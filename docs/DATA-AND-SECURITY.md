...
- **When calls occur**: once at app launch (one all-account batch that seeds the cache), then on a tiered background schedule - the active account every 5 minutes, inactive accounts every 30 minutes - and after every account switch as one all-account batch. Opening the menu never triggers a fetch; it renders the cached rows. Each batch is one request per account, each authenticated with that account's own saved credential. Failed rows respect a retry cooldown (including the server's `Retry-After` hint for 429 responses), a request is skipped while one is already in flight for the same account, and network concurrency is capped at three in-flight requests per batch. The background auth-sync timer never triggers usage requests; usage is a deliberate low-frequency schedule (5/30 minutes per account), never a continuous poller.
