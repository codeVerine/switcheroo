...
- **When calls occur**: at app launch and each time the menu is opened, if the last result is over a minute old, Switcheroo fetches usage for all saved accounts as a batch - one request per account, each authenticated with that account's own saved credential. A request is skipped while one is already in flight for the same account; never a polling service.
