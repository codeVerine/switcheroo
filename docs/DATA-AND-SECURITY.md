...
- **When calls occur**: at app launch, each time the menu is opened (if the last result is over a minute old), and after every account switch, as one all-account batch - one request per account, each authenticated with that account's own saved credential. Failed rows respect a retry cooldown (including the server's `Retry-After` hint for 429 responses), a request is skipped while one is already in flight for the same account, and network concurrency is capped at three in-flight requests per batch. The background auth-sync timer never triggers usage requests; never a polling service.
