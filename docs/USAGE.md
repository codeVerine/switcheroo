...
| Control | Behavior |
| --- | --- |
| Open menu | Reloads config and active status from disk and renders each account's cached usage; opening the menu never fetches usage. |
...
- **Caching and retries**: results are kept in memory; the active account's result is considered fresh for 5 minutes and inactive accounts' for 30 minutes, so a request is skipped while its tier is fresh or one is already in flight for the same account. Failed rows wait out a short cooldown (honoring the server's `Retry-After` hint when provided) before being retried by the next scheduled refresh. The per-batch request limit is documented in [Data & Security](/docs/DATA-AND-SECURITY.md). No usage requests ever originate from the background auth-sync timer.
...
- Usage rows refresh in the background on a tiered schedule: the active account every 5 minutes, inactive accounts every 30 minutes, in addition to the one-time launch seeding and the all-account refresh after every switch.
